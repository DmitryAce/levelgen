...
def draw_graph(states, connections):
    ...
    for k in connections:
        dot.edge(str(k[0]), str(k[1]), label=k[2])
    ...
    while queue:
        ...
        if board[y][x] == "E" and is_subset(win_req, current):
            ...
            if farthest_exit == (x, y):
                ...
                connections.append((current['state'], STATECNT, "fake"))
                continue
            else:
                STATECNT += 1
                board[y][x] = " "
                ...
                reached.append(cur_state)
                connections.append((current['state'], STATECNT, collectables_dict.get("E")))
                continue
        ...
        if board[y][x] == "X" and is_subset(win_req, current):
            ...
            states.append(cur_state)
            reached.append(cur_state)
            connections.append((current['state'], STATECNT, collectables_dict.get("X")))
            continue
        ...
        if is_subset(win_req, current) and EXITTYPE == 0:
            ...
            states.append(cur_state)
            reached.append(cur_state)
            connections.append((current['state'], STATECNT, 'escaped'))
            continue
        ...
        if board[y][x] in collectables:
            ...
            connections.append((current['state'], STATECNT, collectables_dict.get(loot)))
            continue
        ...
        for dx, dy in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
            ...
            for r in reached:
                if is_subset(new_state, r):
                    #connections.append((current['state'], r['state'], collectables_dict.get((dx, dy))))
                    exist = True
                    break
            ...
            if 0 <= new_state['y'] < len(board) and 0 <= new_state['x'] < len(board[0]):
                ...
                queue.append(cur_state)
                reached.append(current)
                connections.append((current['state'], STATECNT, collectables_dict.get((dx, dy))))
